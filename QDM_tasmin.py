"""
This script performs QDM bias correction of CMIP6 minimum temperature outputs.
...
#Observational ERA5 datasets
file_path_obs = f'ERA5_MED_{var}.nc'
ds_obs_var = xr.open_dataset(file_path_obs)
file_path_obs_mean = f'ERA5_MED_{var_mean}.nc'
ds_obs_mean =xr.open_dataset(file_path_obs_mean)

# CNN-based regridded GCM datasets for the selected model
os.chdir(f'Models_MED_regridded/{var}')
file_path_mod = f"{mod_name}_{var}_regridded.nc"
ds_mod_var = xr.open_dataset(file_path_mod)
os.chdir('../..')

os.chdir(f'Models_MED_regridded/{var_mean}')
file_path_mod_mean = f"{mod_name}_{var_mean}_regridded.nc"
ds_mod_mean = xr.open_dataset(file_path_mod_mean)
os.chdir('../..')

time_slice_his = slice('1985-01-01', '2014-12-31')
time_slice_modp = slice('2015-01-01', '2100-12-31')

ds_mod_var = ds_mod_var.sel(time_hist=time_slice_his)

ds_obs=ds_obs_var.copy(deep=True)
ds_obs[f'{var}_ERA5']=ds_obs_mean[f'{var_mean}_ERA5']-ds_obs_var[f'{var}_ERA5']

ds_mod=ds_mod_var.copy(deep=True)
ds_mod[f'{var}_hist']=ds_mod_mean[f'{var_mean}_hist']-ds_mod_var[f'{var}_hist']
ds_mod[f'{var}_ssp126']=ds_mod_mean[f'{var_mean}_ssp126']-ds_mod_var[f'{var}_ssp126']
ds_mod[f'{var}_ssp370']=ds_mod_mean[f'{var_mean}_ssp370']-ds_mod_var[f'{var}_ssp370']
...
def quantile_delta_mapping_one_to_one(obs, modh, modp):
    ...
    delta_rolling = delta_xr.rolling(time=60, center=True, min_periods=1).mean().round(5) 
    ...
    corrected_unsorted = corrected_unsorted.compute()
...
obs_30y = ds_obs[f'{var}_ERA5'].sel(time=time_slice_his).load()
modh_30y = ds_mod[f'{var}_hist'].sel(time_hist=time_slice_his).load()

modp_ssp126 = ds_mod[f'{var}_ssp126'].sel(time_ssp=time_slice_modp).load()
modp_ssp370 = ds_mod[f'{var}_ssp370'].sel(time_ssp=time_slice_modp).load()
...
ds_GCM_downscaled_DTM=ds_mod.copy(deep=True)
...
for month in range(1, 13):
    ...
    ds_GCM_downscaled_DTM[f'{var}_hist'].loc[{'time_hist': time_win}] = corrected_y_m
...
for year in range(start_year+half_window_years, end_year-half_window_years + 2):     
    ...
    for month in range(1, 13):
        ...
        ds_GCM_downscaled_DTM[f'{var}_ssp126'].loc[{'time_ssp': time_win}] = corrected_126_y_m
        ds_GCM_downscaled_DTM[f'{var}_ssp370'].loc[{'time_ssp': time_win}] = corrected_370_y_m

os.chdir(f'Models_MED_downscaled/{var_mean}')
file_path_mod_mean_corr = f"{mod_name}_{var_mean}.nc"
ds_GCM_mean_corr = xr.open_dataset(file_path_mod_mean_corr)
os.chdir('../..')
ds_GCM_downscaled=ds_GCM_downscaled_DTM.copy(deep=True)
ds_GCM_downscaled[f'{var}_hist']=ds_GCM_mean_corr[f'{var_mean}_hist']-ds_GCM_downscaled_DTM[f'{var}_hist']
ds_GCM_downscaled[f'{var}_ssp126']=ds_GCM_mean_corr[f'{var_mean}_ssp126']-ds_GCM_downscaled_DTM[f'{var}_ssp126']
ds_GCM_downscaled[f'{var}_ssp370']=ds_GCM_mean_corr[f'{var_mean}_ssp370']-ds_GCM_downscaled_DTM[f'{var}_ssp370']
...
encoding = {var_name: {"zlib": True, "complevel": 4} for var_name in ds_GCM_downscaled.data_vars}
