"""
This script performs CNN-based regridding of CMIP6 maximum temperature outputs.
...
var='tasmax' # maximum temperature

######################## CNN training ########################################

# MATLAB v7.3 (HDF5-based) dataset containing ERA5 paired inputs and targets 
# (delta between maximum and mean temperature)
...
class ConstrainLayer(layers.Layer):
    ...
    def call(self, output, input_layer):
        ...
        exp_y = tf.square(output_resh)
        exp_y = tf.expand_dims(exp_y, axis=-1)
        exp_y = tf.cast(exp_y, tf.float32)
        downscaled_y = tf.nn.avg_pool2d(exp_y, ksize=(up_factor, up_factor), strides=(up_factor, up_factor), padding='SAME')
        ...
        const_output = exp_y * upsampled_factor
        constrained_output=tf.reshape(const_output, (tf.shape(input_layer)[0], self.outputSize[0]*self.outputSize[1]))
...
for mod_name in mod_names:
    ...
    ds_GCM_var = xr.open_dataset(f'{mod_name}_{var}_coarse.nc')
    # Move to the directory containing the mean temperature 
    os.chdir(f'../{var_mean}')
    ds_GCM_mean = xr.open_dataset(f'{mod_name}_{var_mean}_coarse.nc')
    # Move to the directory containing the regridded mean temperature 
    os.chdir(f'../../Models_MED_regridded/{var_mean}')
    ds_GCM_mean_down = xr.open_dataset(f'{mod_name}_{var_mean}_regridded.nc')
    
    var_hist=ds_GCM_var[f'{var}_hist']-ds_GCM_mean[f'{var_mean}_hist']
    var_hist=var_hist.clip(min=0.1)
    var_hist_down= CNN.predict(var_hist)
    var_hist_down = np.reshape(var_hist_down, (var_hist_down.shape[0], outputSize[0], outputSize[1]))
    var_hist_down = ds_GCM_mean_down[f'{var_mean}_hist'] + var_hist_down
    
    X_ssp126=ds_GCM_var[f'{var}_ssp126']-ds_GCM_mean[f'{var_mean}_ssp126']
    X_ssp126=X_ssp126.clip(min=0.1)
    ...
    var_ssp126_down = ds_GCM_mean_down[f'{var_mean}_ssp126'] + var_ssp126_down
    
    X_ssp370=ds_GCM_var[f'{var}_ssp370']-ds_GCM_mean[f'{var_mean}_ssp370']
    X_ssp370=X_ssp370.clip(min=0.1)
    ...
    var_ssp370_down = ds_GCM_mean_down[f'{var_mean}_ssp370'] + var_ssp370_down
    ...
    ds_GCM_regridded = ds_GCM_regridded.assign_coords(time_hist=ds_GCM_var['time_hist'])
    ds_GCM_regridded = ds_GCM_regridded.assign_coords(time_ssp=ds_GCM_var['time_ssp'])
    ds_GCM_regridded = ds_GCM_regridded.assign({f"{var}_hist": (("time_hist", "lat", "lon"), var_hist_down.data)})
    ds_GCM_regridded = ds_GCM_regridded.assign({f"{var}_ssp126": (("time_ssp", "lat", "lon"), var_ssp126_down.data)})
    ds_GCM_regridded = ds_GCM_regridded.assign({f"{var}_ssp370": (("time_ssp", "lat", "lon"), var_ssp370_down.data)})
    ds_GCM_regridded = ds_GCM_regridded.assign(time_hist=ds_GCM_var['time_hist'])
    ds_GCM_regridded = ds_GCM_regridded.assign(time_ssp=ds_GCM_var['time_ssp'])
